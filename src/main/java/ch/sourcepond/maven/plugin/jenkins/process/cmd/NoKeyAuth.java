 ...
 *
 */
@Named(NoKeyAuth.NO_KEY_AUTH_SWITCH)
@Singleton
final class NoKeyAuth extends Token {
	static final String NO_KEY_AUTH_SWITCH = "-noKeyAuth";

	/**
	 * @param pNext
	 */
	@Inject
	NoKeyAuth(@Named(NO_CERTIFICATE_CHECK_SWITCH) final CommandToken pNext) {
		super(pNext);
	}
	...
	public void doVisitToken(final List<String> pTokens, final Config pConfig) {
		pTokens.add(NO_KEY_AUTH_SWITCH);
	}
}
