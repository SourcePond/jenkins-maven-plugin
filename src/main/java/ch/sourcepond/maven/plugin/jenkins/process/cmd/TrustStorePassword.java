 ...
 *
 */
@Named(TrustStorePassword.TRUST_STORE_PASSWORD)
@Singleton
final class TrustStorePassword extends Token {
	static final String TRUST_STORE_PASSWORD = "-Djavax.net.ssl.trustStorePassword";

	/**
	 * @param pNext
	 */
	@Inject
	TrustStorePassword(@Named(JAR_SWITCH) final CommandToken pNext) {
		super(pNext);
	}
	...
	protected boolean isResponsible(final Config pConfig) {
		return pConfig.isSecure()
				&& pConfig.getTrustStorePasswordOrNull() != null;
	}
	...
	public void doVisitToken(final List<String> pTokens, final Config pConfig) {
		addSingleParameter(pTokens, TRUST_STORE_PASSWORD,
